...
export const SigningFlow: React.FC = () => {
  ...
  const handleParsePSBT = async () => {
    ...
    setError('');
    try {
      const details = await rpc.request.parsePsbt({
  ...
  const handleSignPSBT = async () => {
    ...
    setError('');
    try {
      const result = await rpc.request.signPsbt({
        psbtData: psbtInput.trim(),
        format: psbtFormat,
      });
            ...
            <div className="space-y-6">
                  ...
                  {psbtDetails.outputs.map((output, idx) => (
                    <div
                      ...
                      className={`bg-hacker-black/50 border p-3 ${
                        output.isChange ? 'border-terminal-green/50' : 'border-bitcoin-orange/50'
                      }`}
                    >
                      <div className="flex justify-between items-start">
                        ...
                        <span className={`font-mono ${output.isChange ? 'text-terminal-green' : 'text-bitcoin-orange'}`}>
              ...
              <div className="bg-hacker-black/50 border border-muted-gray p-4 space-y-2">
                <div className="flex justify-between">
                  ...
                  <span className="text-hacker-white font-mono">{formatBtc(psbtDetails.totalInput)} BTC</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-gray font-mono">Total Output:</span>
                  <span className="text-hacker-white font-mono">{formatBtc(psbtDetails.totalOutput)} BTC</span>
                </div>
                <div className="flex justify-between border-t border-muted-gray/30 pt-2">
                  ...
                  <span className="text-bitcoin-orange font-mono">
                    {formatBtc(psbtDetails.fee)} BTC ({psbtDetails.feeRate.toFixed(1)} sat/vB)
                  </span>
                </div>
              </div>
        ...
        return (
          <Card>
            ...
            <div className="space-y-6">
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  ...
                  <Button
                    ...
                    onClick={() => handleCopyToClipboard(signedResult.signedPsbt)}
                  >
                    Copy
                  </Button>
                </div>
                <textarea
                  value={signedResult.signedPsbt}
                  readOnly
              ...
              {/* Raw TX if finalized */}
              {signedResult.isFinalized && signedResult.transactionHex && (
                <div>
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <Button
                      variant="secondary"
                      className="text-xs px-2 py-1"
                      onClick={() => handleCopyToClipboard(signedResult.transactionHex!)}
                    >
                      Copy
                    </Button>
                  </div>
                  <textarea
                    value={signedResult.transactionHex}
                    readOnly
